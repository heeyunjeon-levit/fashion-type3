...
<body>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {{
            ...
            // Prevent body/background scroll, allow sheet scroll
            let touchStartY = 0;
            ...
            document.body.addEventListener('touchmove', function(e) {{
                // Only prevent scroll if touching outside the sheet
                const target = e.target;
                const isInsideSheet = sheetContent && sheetContent.contains(target);
            ...
            // Ensure sheet content is scrollable
            if (sheetContent) {{
                sheetContent.style.webkitOverflowScrolling = 'touch';
                sheetContent.style.overflowY = 'auto';
            }}
            
            // Handle horizontal product scrolling
            const productsScrolls = document.querySelectorAll('.products-scroll');
            productsScrolls.forEach(function(productsScroll) {{
                let startX = 0;
                let scrollLeft = 0;
